            ...
            with tf.Session(config=config) as sess:
                ...
                for step in range(cfgs.MAX_ITERATION):
                    ...
                    if step > 0 and step == cfgs.MAX_ITERATION - 1:
                        ...
                        # self.checkpoint_path = os.path.join(save_dir, '{}_'.format(
...
if __name__ == '__main__':
    ...
    img_dir = cfgs.ROOT_PATH + f"/data/increment_data/test_data/JPEGImages"
    xml_dir = cfgs.ROOT_PATH + f"/data/increment_data/test_data/Annotations"

    data = BaseDataSource(data_type="test")
    data.x = [os.path.join(img_dir, img_name) for img_name in os.listdir(img_dir)]
    data.y = [os.path.join(xml_dir, xml_name) for xml_name in os.listdir(xml_dir)]

    estimator.evaluate(data)
    # predict_dict = estimator.predict(data.x)
    # print(f1_score(data.y, predict_dict, data=data))
