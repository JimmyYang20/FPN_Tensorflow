...
def clip_obj_imgs(src_img, boxes, classes, scores, des_folder):
  ...
  while ii < box_num:
    ...
    # plt.imshow(img)
    # plt.show()
    clip_path = os.path.join(des_folder, '%s-%d_%.3f.jpeg' % (classes[ii], ii, scores[ii]))
...
def detect_img(file_paths, des_folder, det_th, h_len, w_len, show_res=False):
  with tf.Graph().as_default():
    ...
    restorer, restore_ckpt = restore_model.get_restorer(True,"./output/layer/res101_trained_weights/v1_layer/layer_49999model.ckpt")
    ...
    with tf.Session(config=config) as sess:
      ...
      for img_path in file_paths:
        ...
        cv2.imwrite(des_folder + '/{}_fpn.jpeg'.format(img_path.split('/')
                                                      [-1].split('.')[0]), img_np)
...
def parse_args():
  ...
  parser.add_argument('--src_folder', dest='src_folder',
                      help='images path',
                      default='./Test2018_2020', type=str)
  parser.add_argument('--des_folder', dest='des_folder',
                      help='output path',
                      default='./image_detect_result', type=str)
  parser.add_argument('--weights', dest='model_weights',
                      help='model path',
                      default="./output/layer/res101_trained_weights/v1_layer/layer_49999model.ckpt",
                      type=str)
